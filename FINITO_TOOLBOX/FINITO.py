...
def MEF2D(**kwargs):
    ...
    * Under development
    ...
    DICTIONARY = kwargs.get('DICTIONARY')
    if FILENAME:
        [N_NODES, N_MATERIALS, N_THICKNESS, N_ELEMENTST3, N_ELEMENTST6, N_ELEMENTST10, N_FORCES, N_PRESSURES, N_DISPLACEMENTS, TIPO_ANALISE, TYPE_ELEMENT, TYPE_SOLUTION, GRAU_INT, COORDINATES, MATERIALS, THICKNESS, ELEMENTS, NODAL_EXTERNAL_LOAD, PRESCRIPTIONS] = FINITO_CL.GET_VALUE_FROM_TXT_MEF2D_FINITO(FILENAME)
    else:
        pass
    # Creating the algorithm's internal parameters
    N_DOFSNODE, N_NODESELEMENT, DOFS_ACTIVE, DOFS_LOCAL, AUX_1, AUX_2, N_DOFSELEMENT = FINITO_CL.INDEX_ASSEMBLY(TYPE_ELEMENT)
    ...
    NUM_INT = NUMERICAL_INTEGRATION(TYPE_INTEGRATION)
    for I_ELEMENT in range(N_ELEMENTS):
